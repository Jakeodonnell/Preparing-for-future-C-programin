...
REM *
REM *----------------------------------------------------------------------------------------------------------------------
REM * If you don't have administrative rights to modify program Files on the Wirndows computer you can also do the following
REM *
REM * 1. Copy this batch file and bossac.exe into the Solution folder of your Atmel Studio project
REM *
REM * 2. Configure an 'External Tool' in Atmel Studio 6 (Tools -> External Tools...).
REM *
REM * 2.1 Configure a debug build command
REM *     Titel: BossacArduinoDue(Debug)
REM *     Command: $(SolutionDir)\BossacArduinoDue.bat
REM *     Arguments: "$(SolutionDir)\bossac.exe"  "$(ProjectDir)\Debug\$(ProjectFileName).bin"
REM *     Checkbox "Use Output Window".
REM *
REM * 2.2. Configure a release build command
REM *      Titel: BossacArduinoDue(Release)
REM *      Command: $(SolutionDir)\BossacArduinoDue.bat
REM *      Arguments: $(SolutionDir)\bossac.exe"  "$(ProjectDir)\Release\$(ProjectFileName).bin"
REM *      Checkbox "Use Output Window".
REM *
REM * 3. Call 'External Tool' in Atmel Studio 6.
REM *    Tools -> BossacArduinoDue(Debug) for Debug Build or BossacArduinoDue(Release) for Release Build
...
REM Wait X second for memory on Arduino Due is erased.
REM IMPORTANT! It's necessary to execute this delay in its own command window.
REM Executing this directly from Atmel Studio won't work! /M.Beckius 2014-11-03
ECHO Wait for memory on Arduino Due is erased...
start /wait timeout /t %WAIT_ERASED%
REM Alternate methods for creating a delay:
REM 	PING -n %WAIT_ERASED% 127.0.0.1>NUL
REM 	NETSH Diag Ping Loopback

REM Execute bossac.exe
ECHO Execute bossac with command line:
ECHO "%BOSSACPATH%" --port=%COMPORT% -U false -e -w -v -b -R "%BINFILE%"
"%BOSSACPATH%" --port=%COMPORT% -U false -e -w -v -b -R "%BINFILE%"
